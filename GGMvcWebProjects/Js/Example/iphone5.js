...
$(function () {
    var zoom = mfun.UrlRequestParams("zoom"); // zoom表示缩放，例如 /?zoom=0.15
    var zoomStyle = (zoom ? zoom * 100 + "%" : "100%");
    $("body").css({ "zoom": zoomStyle });
    $(".iphone").show(); //防止页面的缩放闪烁
});
...
/**
 * 原例网址 http://carrisa.github.io/iphone/ 
 */
$(function () {
    clock();
    compass();
...
function clock() {
    for (var i = 1; i <= 12; i++) {
        $("#clock ul li:nth-child(" + i + ")").css({
            "-webkit-transform": "rotate(" + (i - 1) * 30 + "deg)", "-o-transform": "rotate(" + (i - 1) * 30 + "deg)", "transform": "rotate(" + (i - 1) * 30 + "deg)",
            "-ms-transform": "rotate(" + (i - 1) * 30 + "deg)", "-moz-transform": "rotate(" + (i - 1) * 30 + "deg)"
        });
        $("#clock ul li:nth-child(" + i + ") span").css({
            "-webkit-transform": "scale(0.6) rotate(" + (1 - i) * 30 + "deg)", "-moz-transform": "scale(0.6) rotate(" + (1 - i) * 30 + "deg)",
...
function update() {
    ...
    $("#phone-time").html(h + ":" + m);
    $("#hour").css({
        "-webkit-transform": "rotate(" + hRadius + "deg)", "-moz-transform": "rotate(" + hRadius + "deg)",
        "-ms-transform": "rotate(" + hRadius + "deg)", "-o-transform": "rotate(" + hRadius + "deg)", "transform": "rotate(" + hRadius + "deg)"
    });
    $("#minute").css({
        "-webkit-transform": "rotate(" + mRadius + "deg)", "-moz-transform": "rotate(" + mRadius + "deg)",
        "-ms-transform": "rotate(" + mRadius + "deg)", "-o-transform": "rotate(" + mRadius + "deg)", "transform": "rotate(" + mRadius + "deg)"
    });
    $("#second").css({
        "-webkit-transform": "rotate(" + sRadius + "deg)", "-moz-transform": "rotate(" + sRadius + "deg)",
...
function compass() {
    for (var i = 0; i < 24; i++) {
        ...
        $("#compass ul li:nth-child(" + (i + 1) + ")").css({
            "-webkit-transform": "rotate(" + i * 7.5 + "deg)", "-moz-transform": "rotate(" + i * 7.5 + "deg)",
...
function safari() {
    for (var i = 0; i < 36; i++) {
        ...
        var a = (i * 2) + 1, b = i * 2;
        $("#safari ul li:nth-child(" + a + ")").css({
            "-webkit-transform": "rotate(" + a * 10 + "deg)", "-moz-transform": "rotate(" + a * 10 + "deg)",
            "-ms-transform": "rotate(" + a * 10 + "deg)", "-o-transform": "rotate(" + a * 10 + "deg)", "transform": "rotate(" + a * 10 + "deg)"
        });
        $("#safari ul li:nth-child(" + b + ")").css({
            "-webkit-transform": "rotate(" + b * 10 + "deg)", "-moz-transform": "rotate(" + b * 10 + "deg)",
...
function settings() {
    for (var i = 0; i <= 24; i++) {
        ...
        $("#settings .round li:nth-child(" + (i + 1) + ")").css({
            "-webkit-transform": "rotate(" + a + "deg)", "-moz-transform": "rotate(" + a + "deg)",
